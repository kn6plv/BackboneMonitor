...
class Node {
    ...
    async getHealth(root) {
        ...
        if (!root) {
            root = await (await this.getSite()).getRootNode();
        }
    ...
    async updateBasics(sysinfo) {
        ...
        await db.query(
            `INSERT INTO nodes_basics (timestamp, name, description, lat, lon, hardware, firmware, mac, ssid, channel, frequency, bandwidth) VALUES(datetime("now"),"${this.name}", "${HtmlEntities.decode(sysinfo.node_details.description)}", ${sysinfo.lat}, ${sysinfo.lon}, "${sysinfo.node_details.model}", "${sysinfo.node_details.firmware_version}", "-", "${sysinfo.meshrf.ssid || "OFF"}", ${sysinfo.meshrf.channel || 0}, ${freq}, ${sysinfo.meshrf.chanbw || 0});`
        );
    }
